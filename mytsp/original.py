...
class TSP(object):
    ...
    def load_Citys2(self, file='mytsp/china.csv', delm=';'):
        # 中国34城市经纬度
        data = pd.read_csv(file, delimiter=delm, header=None).values
        self.citys = data[:, 1:]
    ...
    def draw_citys_name(self, gen, size=5):
        ...
        tsp.dw.bound_y = [np.min(tsp.citys[:, 1]), np.max(tsp.citys[:, 1])]
        """
        tsp.dw.set_xybound(tsp.dw.bound_x, tsp.dw.bound_y)  #画坐标轴及刻度
        for i in range(m):
            c = gen[i]
            best_icity = tsp.citys[c]
            tsp.dw.draw_text(best_icity[0], best_icity[1], tsp.citys_name[c], 10)  # float , float, string
